...
function infolist(){
	$arr = array();
	$data=db_load_sql("select infoid,info_title,info_content,info_onoff,info_order,info_addtime,info_pic from [dbpre]info");
	foreach ($data as $value) {
		$i=$value['infoid'];
		array_push($arr,array(
		'id' => $i,
		'info_title'=> "<input type='text' class='title' id='info_title-".$i."' placeholder='".$value["info_title"]."'  value='".$value["info_title"]."'>",
		
		'info_order'=> "<input type='number' class='order' id='info_order-".$i."' placeholder='".$value["info_order"]."'  value='".$value["label_order"]."'>",
		'info_onoff'=> "<button type='button'  onclick=setcol('info_onoff',".abs($value['info_onoff']-1).",'".$i."') id='".$i."info_onoff' class='btn btn-".$value["info_onoff"]."  dim'><i class='fa fa-toggle-on'></i></button>",	
		'edit'=> "<button type='button' class='btn btn-info dim' onclick=opennew('?act=info&id=".$i."')><i class='fa fa-pencil'></i></button> <button type='button'  onclick=delid('".$i."','info')  class='btn btn-danger dim'><i class='fa fa-times'></i></button>"
