...
function aboutlist(){
	...
	foreach ($data as $value) {
		...
		array_push($arr,array(
			...
			'edit' => '<button type="button" class="btn btn-info dim div-block" onclick="opennew(\'?act=about&aid='.$value['aid'].'\')"><i class="fa fa-pencil"></i></button><button type="button" class="btn btn-info dim div-hidden" onclick="opennewfull(\'?act=about&aid='.$value['aid'].'\')"><i class="fa fa-pencil"></i></button> '		
			));
		}
	}
	return tojson($arr);
...
function sortlist($pid){
	$arr = array();
	$count	=	db_count("sort");	
	$data=db_load_sql('select t.*,model_type,model_name,model_table,(select count(*) from [dbpre]sort where s_pid=t.sid and (s_onoff=0 or s_onoff=1)) as c from [dbpre]model as a,[dbpre]sort t where s_type=model_type and (s_onoff=0 or s_onoff=1) and s_pid='.$pid.' and s_lid='.get_session("lid").'  order by s_order asc , sid asc');
	foreach ($data as $value) {
...
function contentlist(){
	...
	if (get_session("admingid")==9) {
		arr_add($where,'c_mid',get_session("adminid"));
	}
	...
	foreach ($data as $value) {
		...
		$edit ='<button type="button" class="btn btn-info dim div-block" onclick="opennew(\'?act=content&cid='.$i.'\')" ><i class="fa fa-pencil"></i></button><button type="button" class="btn btn-info dim div-hidden" onclick="opennewfull(\'?act=content&cid='.$i.'\')" ><i class="fa fa-pencil"></i></button>';
		$sms = '';
		if (get_session('lid')==3) {
			$sms ='<button type="button" class="btn btn-danger dim" onclick="sendsms('.$i.',\'content\')"  onmouseover=layer.tips("短信通知",this)><i class="fa fa-comment"></i></button>';
			// $sms ='<button type="button" class="btn btn-danger dim" onclick=openonoff("send.php?id=1")  onmouseover=layer.tips("短信通知",this)><i class="fa fa-comment"></i></button>';
		}
		...
		array_push($arr,array(
			...
			'c_onoff'=> get_session("admingid")==9 ? '<button type="button" onclick=setcol2("c_onoff",'.abs($value['c_onoff']-1).',"'.$i.'") id="'.$i.'c_onoff" class="btn btn-'.$value['c_onoff'].'  dim"><i class="fa fa-toggle-on"></i></button>' : '<button type="button" onclick=setcol("c_onoff",'.abs($value['c_onoff']-1).',"'.$i.'") id="'.$i.'c_onoff" class="btn btn-'.$value['c_onoff'].'  dim"><i class="fa fa-toggle-on"></i></button>',
			'istop'=>			
			...
			'edit' => conf("isdel")==0 ? $edit.$sms.'<button type="button" class="btn btn-warning dim" onclick="remove('.$i.',\'content\')"><i class="fa fa-trash"></i></button>' : $edit.$sms.'<button type="button" class="btn btn-danger dim" onclick="delid('.$i.',\'content\')" ><i class="fa fa-times"></i></button>'
			));
	}
...
function userlist(){
	$arr = array();$whereStr='';
	$data=db_load_sql('select * from [dbpre]user_txl');	
	foreach ($data as $value) {
		$value=array_change_key_case($value);
		$value['leader'] == 0 ? $status = "<i class='fa fa-times text-danger'></i>"	: 	$status = "<i class='fa fa-check text-primary'></i>" ;
		$i=$value['id'];
		array_push($arr,array(
		'id' => $i,
		...
		'logins'=>$value["logins"],
		'leader'=> $status,	
		'edit'=> "<button type='button' class='btn btn-info dim' onclick=opennew('?act=user_txl&id=".$i."')><i class='fa fa-pencil'></i></button> <button type='button'  onclick=delid('".$i."','user_txl')  class='btn btn-danger dim'><i class='fa fa-times'></i></button>"
...
function regionlist(){
	$arr = array();
	$pid=getform('pid','get','','0');
	$search=getform('search','post');
	if($search){
		$data=db_load('region',['title,zipcode,telcode,zonecode'=>['LIKE'=>$search]]);
	}else{
		$data=db_load('region',['pid'=>$pid]);
	}
	
	foreach ($data as $value) {
		$i=$value['id'];
		$next=$value['rid']<10000 ? "<a type='button' class='btn' href='?act=regionlist&pid=".$value['rid']."'><i class='fa fa-links'></i> 查看下级</a>" : "";
		array_push($arr,array(
		  'id' => $i,
		  'title'=> "<input type='text' id='title-".$i."' placeholder='".$value['title']."' value='".$value['title']."'>".$next,
		  'entitle'=> "<input type='text' id='entitle-".$i."' placeholder='".$value['entitle']."' value='".$value['entitle']."'>",
		  'zipcode'=> "<input type='text' class='title' id='zipcode-".$i."' placeholder='".$value['zipcode']."' value='".$value['zipcode']."'>",
		  'telcode'=> "<input type='text' class='title' id='telcode-".$i."' placeholder='".$value['telcode']."' value='".$value['telcode']."'>",
		  'zonecode'=> "<input type='text' class='title' id='zonecode-".$i."' placeholder='".$value['zonecode']."' value='".$value['zonecode']."'>",
		  'onoff'=> "<button type='button'  onclick=setcol('onoff',".abs($value['onoff']-1).",'".$i."')  id='".$i."onoff' class='btn btn-".$value['onoff']."  dim'><i class='fa fa-toggle-on'></i></button>",
		  'order'=> "<input type='number' class='order'  min='0' max='99' id='r_order".$i."' placeholder='".$value['r_order']."'  value=".$value['r_order'].">",
		  'edit'=>  "<button type='button' class='btn btn-success  dim' onclick=opennew('?act=region&depth=".($value['depth']+1)."&ptitle=".$value['title']."&pid=".$value['rid']."')><i class='fa fa-plus'></i> </button> <button type='button' class='btn btn-info dim' onclick=opennew('?act=region&id=".$value['id']."')><i class='fa fa-pencil'></i> </button>"
		));
	}
...
function customlist(){
	...
	if(db_table($value['customtype'])=='content'){
		$editlink='?act=custom&customid='.$i;
	}else{
		$editlink='?act='.$value['customtype'].'custom&customid='.$i;
	}
	array_push($arr,array(	
		'id' => $i,	
...
function filelist($str){
	...
	foreach ($data as $value) {
		...
		switch ( $value['ext']) {
			...
			$edit='<a href="'.$value['url'].'" class="btn btn-default dim" target="_blank"><i class="fa fa-external-link"></i></a> <a href="javascript:void(0)" class="btn btn-success dim" onclick=opennew(\'?act=templateedit&folder='.$value['url'].'\')><i class="fa fa-pencil"></i></a>'.$edit;
			$name='<a href="javascript:void(0)" onclick=opennew(\'?act=templateedit&folder='.$value['url'].'\')><i class="fa fa-edit"></i>'.$value['name'].'</a>';
			$ext='Text';
			$size=$value['size'];
			break;
			case 'html':case 'tpl':
			$edit= '<a href="javascript:void(0)" class="btn btn-success dim" onclick=opennew(\'?act=templateedit&folder='.$value['url'].'\')><i class="fa fa-pencil"></i></a>'.$edit;
			$name=  $str=='html'  ? '<a href="'.$value['url'].'" target="_blank">'.$value['name'].'</a>' :'<a href="javascript:void(0)" onclick=opennew(\'?act=templateedit&folder='.$value['url'].'\')><i class="fa fa-edit"></i>&nbsp;'.$value['name'].'</a>';
			$ext='Html';
			$size=$value['size'];
			break;
...
function noticelist(){
	$arr = array();
	$data=db_load_sql("select * from [dbpre]notice");
	foreach ($data as $value) {
		$i=$value['id'];
		array_push($arr,array(
		'id' => $i,
		...
		'order'=> "<input type='number' class='order' id='order-".$i."' placeholder='".$value["order"]."'  value='".$value["order"]."'>",
		'onoff'=> "<button type='button'  onclick=setcol('onoff',".abs($value['onoff']-1).",'".$i."') id='".$i."onoff' class='btn btn-".$value["onoff"]."  dim'><i class='fa fa-toggle-on'></i></button>",	
		'edit'=> "<button type='button' class='btn btn-info dim' onclick=opennew('?act=notice&id=".$i."')><i class='fa fa-pencil'></i></button> <button type='button'  onclick=delid('".$i."','notice')  class='btn btn-danger dim'><i class='fa fa-times'></i></button>"
...
function navlist(){
	...
	foreach ($data as $value) {
		$i=$value['id'];
		$value['n_type'] == 1 ? $n_type = '用户菜单' : $n_type = '系统菜单' ;
		array_push($arr,array(
			'id' => $value['id'],
			'n_name'=> '<input type="text" class="title" id="n_name-'.$i.'" placeholder="'.$value['n_name'].'" value="'.$value['n_name'].'">',	
			'n_type'=> $n_type,	
			'n_icon'=> '<i class="mdi '.$value['n_icon'].'"></i>',		
			'n_url'=> '<input type="text" class="title" id="n_url-'.$i.'" placeholder="'.$value['n_url'].'" value="'.$value['n_url'].'">',		
			'n_order'=> '<input type="number" class="order" id="n_order-'.$i.'" placeholder="'.$value['n_order'].'" value="'.$value['n_order'].'">',
			'n_onoff'=> '<button type="button"  onclick=setcol("n_onoff",'.abs($value['n_onoff']-1).','.$i.')  id="'.$i.'.n_onoff"  class="btn btn-'.$value["n_onoff"].' dim"><i class="fa fa-toggle-on"></i></button>',	
			'edit' => '<button type="button" class="btn btn-info dim" onclick="opennew(\'?act=nav&id='.$value['id'].'\')"><i class="fa fa-pencil"></i></button> <button type="button" class="btn btn-danger dim" onclick="delid('.$value['id'].',\'nav\')"><i class="fa fa-times"></i></button>'
			));
	}	
	return tojson($arr);
}
