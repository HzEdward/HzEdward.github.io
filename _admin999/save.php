...
switch ($act) {
	...
	case 'getuser':		return up_getuser(); 		break;	
...
function set_col(){
    ...
    if(db_update($table,array(table_id($table)=>$id),array($col=>$colval))){
        if  ($col=="model_onoff") delfiles(RUN_DIR . 'cache/model/','tpl','all');
...
function save_about(){
	...
	$a_pic=$a_pic ?: count($a_picsurl)>0 ? $a_picsurl[0] : '';
	$a_picsurl=count($a_picsurl)==0 ? $a_pic : @implode(",",$a_picsurl);
...
function save_admingroup(){
    ...
    $g_nav=safe_key(getform("g_nav", "post"));		
    empty($g_nav) and $g_nav='all';
	$colarr=array('g_name'=>$g_name,'g_mark'=>$g_mark,'g_desc'=>$g_desc,'g_menu'=>$g_menu,'g_sort'=>$g_sort,'g_nav'=>$g_nav);
	if ($gid==0){
...
function save_content(){
	...
	$c_pid=safe_key(getform("c_pid", "post"));
	$c_user=get_cookie("adminname");
	...
	if ($c_sid > 14 && $c_sid < 19) {//师资队伍
		$c_tag =getFirstChar($c_title);
	}else{
		$c_tag = getform("c_tag", "post");	
		$c_tag=(str_replace('，',',',$c_tag));  check_tag($c_tag);
	}
	...
	$ispic=0;
	if(!empty($c_pic)){
		if(check_file($c_pic,$c_type,$cid)){
	...
	$c_exc=1;
	$colarr=array('c_title'=>$c_title,'c_title2'=>$c_title2,'c_lid'=>get_session("lid"),'c_sid'=>$c_sid,'istop'=>$istop,'isgood'=>$isgood,'ispic'=>$ispic,'c_gid'=>$c_gid,'c_visits'=>$c_visits,'c_type'=>$c_type,'c_color'=>$c_color,'c_link'=>$c_link,'c_tag'=>$c_tag,'c_content'=>$c_content,'c_endtime'=>$c_endtime,'c_addtime'=>$c_addtime,'c_picsurl'=>$c_picsurl,'c_picsname'=>$c_picsname,'c_pic'=>$c_pic,'c_downurl'=>$c_downurl,'c_downname'=>$c_downname,'c_pagename'=>$c_pagename,'c_pagetitle'=>$c_pagetitle,'c_pagekey'=>$c_pagekey,'c_pagedesc'=>$c_pagedesc,'c_exc'=>$c_exc);
	...
	if ($cid==0){
		...
		if(get_session('admingid')<3){
			arr_add($colarr,'c_onoff',1);
		}else{
			arr_add($colarr,'c_onoff',0);
		}
		arr_add($colarr,'c_user',$c_user);
...
function save_user(){
	...
	$password=getform("password", "post") ? getform("password", "post") : 'qqq111';
	...
	$colarr=array('username'=>$username,'truename'=>$truename,'question'=>$question,'answer'=>$answer,'mobile'=>$mobile,'face'=>$face,'u_desc'=>$u_desc,'u_gid'=>$u_gid);
	if($uid==0){
...
function save_sort(){
	...
	$colarr=array('s_type'=>$s_type,'s_pid'=>$s_pid,'s_name'=>$s_name,'s_enname'=>$s_enname,'s_url'=>$s_url,'s_filename'=>$s_filename,'s_pic'=>$s_pic,'s_ico'=>$s_ico,'s_template'=>$s_template,'c_template'=>$c_template,'s_postion'=>$s_postion,'s_gid'=>$s_gid,'s_other2'=>$s_other2,'s_other1'=>$s_other1,'s_title'=>$s_title,'s_key'=>$s_key,'s_addtime'=>$time,'s_desc'=>$s_desc,'s_lid'=>get_session("lid"));
	delfiles(RUN_DIR . 'cache/navlist/','tpl','all');
	if ($sid==0){
		...
		save_path($sid);
		if ($s_type=='about')  db_insert('about',array('a_sid'=>$sid,'a_lid'=>get_session("lid"),'a_name'=>$s_name,'a_order'=>9,'a_onoff'=>1,'a_addtime'=>$time));
...
function add_sort($model,$name,$tid,$pid,$data){
	$time= date('Y-m-d h:i:s',time());
	$colarr=array('s_tid'=>$tid,'s_pid'=>$pid, 's_order'=>9, 's_type'=>$model, 's_name'=>$name,'s_title'=>$name,  's_template'=>$data['model_list_tp'].','.$data['model_list_tp'], 'c_template'=>$data['model_content_tp'].','.$data['model_content_tp'], 's_folder'=>'', 'c_folder'=>'', 's_filename'=>'', 'c_filename'=>'', 's_onoff'=>1,  's_lid'=>get_session("lid"), 's_addtime'=>$time, 's_gid'=>0, );
	$sid=db_insert('sort',$colarr);
	save_path($sid);
	if ($model=='about') db_insert('about',array('a_sid'=>$sid,'a_lid'=>get_session("lid"),'a_name'=>$name,'a_order'=>9,'a_onoff'=>1,'a_addtime'=>$time));
	return $sid;
}
...
function save_slide(){
	...
	if(ifstrin($slideimg,',') && $slideid==0){
		...
		foreach( $slideimgs as $v){
			if($v){
			db_insert('slide',['slidename'=>$slidename,'slidewidth'=>$slidewidth,'slideheight'=>$slideheight,'slideclass'=>$slideclass,'slidelink'=>$slidelink,'slideimg'=>$v,'slidetitle1'=>$slidetitle1,'slidetitle2'=>$slidetitle2,'slidecontent'=>$slidecontent,'slidestyle'=>$slidestyle,'slideorder'=>9,'slideonoff'=>1]);
			}
		}
		returnmsg('json',1,'添加成功');
	}else{
		$colarr=array('slidename'=>$slidename,'slidewidth'=>$slidewidth,'slideheight'=>$slideheight,'slideclass'=>$slideclass,'slidelink'=>$slidelink,'slideimg'=>$slideimg,'slidetitle1'=>$slidetitle1,'slidetitle2'=>$slidetitle2,'slidecontent'=>$slidecontent,'slidestyle'=>$slidestyle);
...
function editfile(){
	...
	$path=rtrim(str_replace($ext,'',$filepath),'.');
    ...
    if(arr_search($file_path,$safe_path)){
	   $file=DOC_DIR.$filepath;
       !(is_file($file)) and returnmsg('json',0,'保存失败，文件不存在'.$file);
    }else{
...
function create_html(){
	...
	if ($folder=='pc'){
		...
		if(get_session("lid")==1){
			$htmldir= SITE_DIR.conf('htmldir');
		}else{
			$htmldir= SITE_DIR.conf('enhtmldir');
		}
...
function save_system(){
	...
	$notice_title=getform('notice_title','post');
	$notice_content=getform('notice_content','post');
	if($safe_type){
...
function save_notice(){
	check_token();
	$id=getform('id','post','',0);
	$title=getform('title','post');
	$addtime=getform('addtime','post');
	$content=getform('content', "post");	
	if ($id==0){		$colarr=array('title'=>$title,'addtime'=>$addtime,'content'=>$content,'order'=>9,'onoff'=>1);
		db_insert('notice',$colarr);
		returnmsg('json',1,'添加成功，是否继续','?act=notice');
	}else{	
		$colarr=array('title'=>$title,'addtime'=>$addtime,'content'=>$content,'edittime'=>date('Y-m-d h:i:s',time()));
		db_update('notice','id='.$id,$colarr);
		returnmsg('json',1,'修改成功');
	}
}
...
function save_nav(){
	check_token();
	$id=getform("id", "post",'',0);
	$n_name=getform("n_name", "post",'nul','json');
	$n_type=getform("n_type", "post",'nul','json');
	$n_icon=getform("n_icon", "post",'nul','json');
	$n_url=getform("n_url", "post");
	$colarr=array('n_name'=>$n_name,'n_type'=>$n_type,'n_icon'=>$n_icon,'n_url'=>$n_url,'n_order'=>9,'n_onoff'=>1);
	if ($id==0){
		arr_add($colarr,'n_order',9);
		arr_add($colarr,'n_onoff',1);
		db_insert('nav',$colarr);
		returnmsg('json',1,'添加成功，是否继续','?act=nav');
	}else{
		db_update('nav','id='.$id,$colarr);
		returnmsg('json',1,'修改成功');
	}
}
...
function up_getuser(){
	...
	//获取用户信息
	//将用户状态status设置为0
	db_update('user_txl','status=1',array('status'=>0));
	$con = dbuser() ; 
	$sql = "SELECT a.*,b.GH,b.SFFCJYSSZLD,b.DZZW FROM t_gxbg_txzlb as a,t_gxjg_jbsjzlb as b  where a.DWH = '0447' and a.GH=b.GH ";
	$result = $con->query($sql);
	// var_dump($result);
	if ($result->num_rows > 0) {
		while ($row = $result->fetch_assoc()) {
		// var_dump($row['DZZW']);
		empty($row['SFFCJYSSZLD']) ? $leader = 0 : $leader = 1 ;
			$colarr=array('username'=>$row['GH'],'truename'=>$row['XM'],'mobile'=>$row['SJHM'],'post'=>$row['DZZW'],'leader'=>$leader,'status'=>1,'sms'=>0);
			if( db_count('user_txl',array('username'=>$row['GH'])) > 0 ){
				db_update('user_txl',array('username'=>$row['GH']),$colarr);
			}else{
				db_insert('user_txl',$colarr);
			}
	...
	mysqli_close($con);
	//删除status为0的用户
	db_delete('user_txl',array('status'=>0));
	returnmsg('json',1,'数据更新成功');
}
?>