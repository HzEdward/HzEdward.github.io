...
function check_sort( $str,$pid ) {
	$thisarr = explode( ',', $str );
	$pidarr = explode( ',', $pid );
	
	foreach ( $thisarr as $value ) {
		$sortname = db_select('sort','s_name',array('sid'=>$value));;
		$checked = in_array( $value, $pidarr ) ? "checked" : '';
		echo "<label class='checkbox-inline'><input class='checkbox' " . $checked . " type='checkbox' name='c_pid[]' value='" . $value . "'>"  . $sortname . "</label>";
	}
}
...
function select_sort( $type, $sid = 0, $pid = 0 ) {
    ...
    if ($type){
        $data=db_load('sort',array('s_onoff'=>1,'s_type'=>$type,'s_lid'=>get_session("lid")),'sid,s_pid,s_name,s_level as level,s_type','1000',array('s_order'=>'asc','sid'=>'desc'));
    }else{
        $data=db_load('sort',array('s_onoff'=>1,'s_lid'=>get_session("lid")),'sid,s_pid,s_name,s_level as level,s_type','1000',array('s_order'=>'asc','sid'=>'desc'));                
    }
	if($data){
	...
	switch ( $num ) {
		...
		case '12':	
			$getCustomClass = "<option value='10' selected>日期时间</option>
								<option value='12' selected>日期</option>
...
function getsortlink( $type, $sid, $filename = '', $outlink = '', $page = 1, $lang='zh' ) {
     if(!defined('WAPPATH')) define('WAPPATH','');
	$runmode = conf( 'runmode' );
	...
	if ( $type == 'links' && !empty( $outlink ) ) {
		if ( strpos( $outlink, '://' ) !== FALSE ) {
			...
			return str_replace( '//', '/', SITE_PATH .WAPPATH . $remark. $outlink );
		}
...
function getletterlink( $name,$sid,$filename = '', $page = 1 ) {
     if(!defined('WAPPATH')) define('WAPPATH','');
	$runmode = conf( 'runmode' );
	if ( $runmode == 0 ) {
		$remark = '?';
	...
	if ( $filename  && strpos( $filename, '{page}' ) === FALSE ) {

		$link = str_replace( '//', '/', SITE_PATH . WAPPATH . $remark . $filename. '_'.$name. $siteext );

	}else{

		$link = SITE_PATH . WAPPATH . $remark . 'list/' . $sid .'_'. $name . $siteext;
	}
	return $link;
}
