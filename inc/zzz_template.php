...
class ParserTemplate {
	...
	function parserCommom( $zcontent ) {
		...
		$zcontent = $this->parserTotal( $zcontent ); // 指定内容
	...
	function parserAbout( $zcontent ) {
		...
		if ( preg_match_all( $pattern, $zcontent, $matches ) ) {
			...
			for ( $i = 0; $i < $count; $i++ ) {
				...
				$pic = empty( $value[ 'a_pic' ] ) ? SITE_PATH . 'images/nopic.png': $value[ 'a_pic' ];
				switch ( $matches[ 1 ][ $i ] ) {
					case 'id':
	...
	function parserBrand( $zcontent ) {
		...
		if ( preg_match_all( $pattern, $zcontent, $matches ) ) {
			...
			for ( $i = 0; $i < $count; $i++ ) {
				$params = parserParam( $matches[ 2 ][ $i ] );
				$pic = empty( $value[ 'b_pic' ] ) ? SITE_PATH . 'images/nopic.png': $value[ 'b_pic' ];
				switch ( $matches[ 1 ][ $i ] ) {
					case 'id':
	...
	function parserContent( $zcontent ) {
		...
		if ( preg_match_all( $pattern, $zcontent, $matches ) ) {
			...
			for ( $i = 0; $i < $count; $i++ ) {
				...
				$pic=  empty( $value[ 'c_pic' ] ) ? SITE_PATH . 'images/nopic.png' : $value[ 'c_pic' ];
				switch ( $matches[ 1 ][ $i ] ) {
					case 'id':
					...
					case 'title':
						...
						break;
					case 'username':
						$zcontent = str_replace( $matches[ 0 ][ $i ], $value[ 'c_mname' ], $zcontent );
						break;	
					case 'pagetitle':
					...
					case 'next':
						...
						$norecord=isset($params['norecord']) ? $params['norecord'] : '没有了' ;
						$zcontent = str_replace( $matches[ 0 ][ $i ], empty( $next ) ? '<a class="newsnext none" href="javascript:void(0)" title="'.$norecord.'">'.$norecord.'</a>' : $next, $zcontent );
						break;
					case 'prev':
						$prev = getprev( $value[ 'c_sid' ], $cid );
						$norecord=isset($params['norecord']) ? $params['norecord'] : '没有了' ;
						$zcontent = str_replace( $matches[ 0 ][ $i ], empty( $prev ) ? '<a class="newsprev none" href="javascript:void(0)" title="'.$norecord.'">'.$norecord.'</a>' : $prev, $zcontent );
						break;
					case 'nexttitle':
					...
					case 'tsid':
						...
						break;
					case 'tslink':
						$sort=db_load_one('sort',['sid'=>$value[ 's_tid' ]]);
						$zcontent = str_replace( $matches[ 0 ][ $i ], getsortlink( $sort[ 's_type' ], $sort[ 'sid' ],  $sort[ 's_filename' ], $sort[ 's_url' ] ), $zcontent );
						break;
					case 'psid':
						$zcontent = str_replace( $matches[ 0 ][ $i ], $value[ 's_pid' ], $zcontent );
						break;
					case 'splink':
						$sort=db_load_one('sort',['sid'=>$value[ 's_pid' ]]);
						$zcontent = str_replace( $matches[ 0 ][ $i ], getsortlink( $sort[ 's_type' ], $sort[ 'sid' ],  $sort[ 's_filename' ], $sort[ 's_url' ] ), $zcontent );
						break;
					default:
						if ( isset( $value[ $matches[ 1 ][ $i ] ] ) ) {
				...
				for ( $i = 0; $i < $count; $i++ ) {
					switch ( $matches[ 1 ][ $i ] ) {
						case 'id':
							...
							break;
						case 'gid':
							$zcontent = str_replace( $matches[ 0 ][ $i ], $data[ 'u_gid' ], $zcontent );
							break;
						case 'name':
	...
	function ParseInTemplate( $zcontent ) {
        $pattern = '/\{zzz:template\s+([\w]+)\s?=\s?([\"\']?)([\w\.\-\/]+)([\"\']?)\s*\}/';
		if ( preg_match_all( $pattern, $zcontent, $matches ) ) {
			$arr = $matches[ 0 ];
			$type = $matches[ 1 ];
			$name = $matches[ 3 ];
			...
			for ( $i = 0; $i < $count; $i++ ) {			
				if($type[ $i ]=='src'){
					$zcontent = str_replace( $arr[ $i ], $this->parserSiteLabel( load_file( TPL_DIR . $name[ $i ] ) ), $zcontent );
				}else if($type[ $i ]=='sid'){
					$sort=db_load_one('sort',['sid'=>$name[ $i ]]);
					$scontent=load_file( TPL_DIR . $sort['s_template']);
					foreach($sort as $k=>$v){
						$key=str_replace('s_','s',$k);
						$scontent=str_replace('{zzz:'.$key.'}',$v,$scontent);
					}				
					$scontent=str_replace('{zzz:'.$v.'}',$v,$scontent);
					$zcontent = str_replace( $arr[ $i ], $this->parserSiteLabel($scontent), $zcontent );
				}else if($type[ $i ]=='tid' || $type[ $i ]=='pid'){
					if($type[ $i ]=='tid'){
						$tid=$name[ $i ] ? $name[ $i ] : G( 'sid' );
						$sorts=db_load('sort',['s_onoff'=>1,'s_tid'=>$tid],'*','100',['s_order'=>'asc','sid'=>'asc']);
					}else{
						$pid=$name[ $i ] ? $name[ $i ] : G( 'sid' );
						$sorts=db_load('sort',['s_onoff'=>1,'s_pid'=>$pid],'*','100',['s_order'=>'asc','sid'=>'asc']);
					}				
					...
					$zcontent = str_replace( $arr[ $i ], $this->parserSiteLabel($tcontent), $zcontent );
				}					
			}
		}		
		if ( preg_match_all( $pattern, $zcontent, $matches ) ) {
	...
	function parserCompanyLabel( $zcontent ) {
		$pattern = '/\{zzz:([\w]+)\}/';
		if ( preg_match_all( $pattern, $zcontent, $matches ) ) {
			$data = db_load_one( 'language', 'l_onoff=1 and l_alias="'.LANGUAGE.'"' );
			...
			for ( $i = 0; $i < count( $qqs ); $i++ ) {
				...
				$qqkfs .= "<li><b>".$contacts[$i]."</b><a target='_blank' href='http://wpa.qq.com/msgrd?v=3&amp;uin=" . $qqs[ $i ] . "&amp;site=qq&amp;menu=yes' class='qq_link'><img border='0' src='http://wpa.qq.com/pa?p=2:" . $qqs[ $i ] . ":51' alt='点击这里给我发消息' title='点击这里给我发消息'></a></li>";
			}
			$count = count( $matches[ 0 ] );
			for ( $i = 0; $i < $count; $i++ ) {
				...
				switch ( $matches[ 1 ][ $i ] ) {
					...
					case 'zid':
						...
						break;  
					case 'bid':
						$zcontent = str_replace( '{zzz:bid}', G( 'bid' ), $zcontent );
						break;  	 
					case 'sid':
	...
	function parserConfigLabel( $zcontent ) {
		...
		if ( preg_match_all( $pattern, $zcontent, $matches ) ) {
			$count = count( $matches[ 0 ] );
			for ( $i = 0; $i < $count; $i++ ) {
				$zcontent = str_replace( $matches[ 0 ][ $i ], conf( $matches[ 1 ][ $i ] ), $zcontent );
			}
		}
		return $zcontent;
	}
			...
			for ( $i = 0; $i < $count; $i++ ) {
				...
				switch ( $label ) {
					case 'about':
						...
						$data = db_load( "about a,sort s", $where);						
						break;
					case 'link':
						$order = array( 'l_order' => $asc, 'lid' => $asc );
								...
								switch ( $label ) {
									case 'about':
										...
										switch ( $matches2[ 1 ][ $j ] ) {
											...
											default:
												...
												isset( $value[ $default_str ] )and $one_html = str_replace( $matches2[ 0 ][ $j ],decode($value[ $default_str ]), $one_html );
												isset( $value[ $matches2[ 1 ][ $j ] ] ) ? $one_html = str_replace( $matches2[ 0 ][ $j ], decode($value[ $matches2[ 1 ][ $j ] ]), $one_html ) : $one_html = str_replace( $matches2[ 0 ][ $j ], '', $one_html );
										}
										break;
									...
									case 'slide': //slidelist
										switch ( $matches2[ 1 ][ $j ] ) {
											case 'name':
											case 'title':
												$one_html = str_replace( $matches2[ 0 ][ $j ], decode($value[ 'slidename' ]), $one_html );
												break;
											case 'spic':
												...
												break;
											case 'pic':
												$one_html = str_replace( $matches2[ 0 ][ $j ], empty( $value[ 'slideimg' ] ) ? SITE_PATH . 'images/nopic.png' : $value[ 'slideimg' ], $one_html );
												break;
											case 'link':
											...
											default:
												...
												isset( $value[ $default_str ] ) ? $one_html = str_replace( $matches2[ 0 ][ $j ], decode($value[ $default_str ]), $one_html ) : $one_html = str_replace( $matches2[ 0 ][ $j ], '', $one_html );
										}
										break;
									case 'tag': //taglist
										switch ( $matches2[ 1 ][ $j ] ) {
									...
									case 'sort': //sortlist
										switch ( $matches2[ 1 ][ $j ] ) {
											...
											case 'enname':
												...
												break;
											case 'pic':
												$one_html = str_replace( $matches2[ 0 ][ $j ], empty( $value[ 's_pic' ] ) ? SITE_PATH . 'images/nopic.png' : $value[ 's_pic' ], $one_html );
												break;
											case 'ico':
												$one_html = str_replace( $matches2[ 0 ][ $j ], empty( $value[ 's_ico' ] ) ? SITE_PATH . 'images/nopic.png' : $value[ 's_ico' ], $one_html );
												break;
											case 'time':
									...
									case 'link':
										switch ( $matches2[ 1 ][ $j ] ) {
											...
											case 'title1':
												...
												break;
											case 'pic':
												$one_html = str_replace( $matches2[ 0 ][ $j ], empty( $value[ 'l_pic' ] ) ? SITE_PATH . 'images/nopic.png' : $value[ 'l_pic' ], $one_html );
												break;
											default:
	...
	function parserNavLabel( $zcontent,$level=1 ) {        
        ...
        $where = array( 's_onoff' => 1,'s_lid' => get_session('s_lid') );
        if ( !empty( $postion ) )arr_add( $where, 's_postion', array( 'LIKE' => $postion ) );
        if ( !empty( $sid ) )arr_add( $where, 's_pid', $sid );
        if ( !empty( $type ) ) {
            if ( $type == 'all' ) {
                $where = array( 's_pid' => 0, 's_onoff' => 1 ,'s_lid' => get_session('s_lid'));
            } else {
						...
						case 'order':
							switch ( $value ) {
								...
								case "price":
									...
									break;
								case "ispx":
								$order = array( 'istop' => $desc,'isgood' => $desc,'c_addtime' => $asc, 'cid' => $asc );
									break;	
								case "rnd":
									$conf=conf('db');
									switch($conf['type']){	
					...
					if ( $count2 ) {
						for ( $j = 0; $j < $count2; $j++ ) { // 循环替换数据						
							...
							switch ( $matches2[ 1 ][ $j ] ) {
								...
								case 'title':
									if(isset($params['len'])){
										$title=leftstr($value[ 'c_title' ],$params['len']/2);
									}else{
										$title=$value[ 'c_title' ];
									}									
									$one_html = str_replace( $matches2[ 0 ][ $j ],$title, $one_html );
								...
								case 'slink':
									...
									break;
								case 'tsid':
									$one_html = str_replace( $matches2[ 0 ][ $j ], $value[ 's_tid' ], $one_html );
									break;
								case 'tslink':
									$sort=db_load_one('sort',['sid'=>$value[ 's_tid' ]]);
									$one_html = str_replace( $matches2[ 0 ][ $j  ], getsortlink( $sort[ 's_type' ], $sort[ 'sid' ],  $sort[ 's_filename' ], $sort[ 's_url' ] ), $one_html );
									break;
								case 'psid':
									$one_html = str_replace( $matches2[ 0 ][ $j ], $value[ 's_pid' ], $one_html );
									break;
								case 'pslink':
									$sort=db_load_one('sort',['sid'=>$value[ 's_pid' ]]);
									$zcontent = str_replace( $matches2[ 0 ][$j  ], getsortlink( $sort[ 's_type' ], $sort[ 'sid' ],  $sort[ 's_filename' ], $sort[ 's_url' ] ), $one_html );
									break;
								case 'sname':
									$one_html = str_replace( $matches2[ 0 ][ $j ], $value[ 's_name' ], $one_html );
									break;
								case 'senname':
									...
									break;
								default:
									isset( $value[ $matches2[ 1 ][ $j ] ] )and $one_html = str_replace( $matches2[ 0 ][ $j ], decode( $value[ $matches2[ 1 ][ $j ] ] ), $one_html );
	...
	public	function parserList( $zcontent ) {
		...
		if ( preg_match_all( $pattern, $zcontent, $matches ) ) {
			...
			for ( $i = 0; $i < $count; $i++ ) {
				...
				$norecord =isset( $norecord ) ? str_replace('　',' ',$norecord) : '很抱歉，没有找到匹配内容！' ;
				if ( $sid )     arr_add( $where, 'c_sid', db_subsort( $sid ) );
				if ( $brand )   arr_add( $where, 'c_brand', $brand );
	...
	function parserSearch( $zcontent ) {
		...
		if ( preg_match_all( $pattern, $zcontent, $matches ) ) {
			...
			for ( $i = 0; $i < $count; $i++ ) {
				...
				$norecord =isset( $norecord ) ? str_replace('　',' ',$norecord) : '很抱歉，没有找到匹配内容！' ;
				$keywords=getform('keys','cookie');	
				$keys =get_searchkey($keywords);					
				if ( $keys ) {					
					$searchcol = getform( 'searchcol', 'cookie' );
					$type      = getform( 'type', 'cookie' );
					$sort      = getform( 'sort', 'cookie' );
					$page      = G('page') ;
					if ( !empty( $searchcol ) ) {
					...
					$where= "c_onoff=1 and (c_title LIKE '%".$keys."%'".$whereor.")";
					if($sort!='' && $sort!='all' )  $where.= " and c_sid in (".$sort.")";
					if($type!='' && $type!='all' )  $where.= " and c_type='". $type."'" ;					
					$GLOBALS[ 'listcount' ]= db_count( 'content', $where);
				...
				$GLOBALS[ 'NOWLINK' ] = $link;
				$zcontent = str_replace( '{zzz:keys}', $keywords, $zcontent );
				$GLOBALS[ "crumbs" ]= array('关键词','<a href="#">' . $keywords . '</a>');
							...
							for ( $j = 0; $j < $count2; $j++ ) { // 循环替换数据
								...
								switch ( $matches2[ 1 ][ $j ] ) {
									...
									case 'downurl':
										...
										break;	
									case 'title':
										$one_html = str_replace( $matches2[ 0 ][ $j ], get_highlight($value[ 'c_title' ],$keywords), $one_html );
										break;
									case 'name':
	...
	function parserTotal( $zcontent ) {
		...
		if ( preg_match_all( $pattern, $zcontent, $matches ) ) {
			...
			//var_dump($jsonarr);
			$jsonarr=$json['data'];
			$jsondata=json_decode($jsonarr,true);
			$chinaTotal=$jsondata['chinaTotal'];	
			//var_dump($chinaTotal);
			$chinaAdd=$jsondata['chinaAdd'];

			$count = count( $matches[ 0 ] );
			for ( $i = 0; $i < $count; $i++ ) {
				//$params = parserParam( $matches[ 1 ][ $i ] );
				if($chinaAdd[ 'importedCase' ]==0){$adddata1='数据待更新';	}else if($chinaAdd[ 'importedCase' ]<0){$adddata1='较昨日<span class="color-1">'.$chinaAdd[ 'importedCase' ].'</span>';}else{$adddata1='较昨日<span class="color-1">+'.$chinaAdd[ 'importedCase' ].'</span>';}
				if($chinaAdd[ 'noInfect' ]==0){$adddata2='数据待更新';	}else if($chinaAdd[ 'noInfect' ]<0){$adddata2='较昨日<span class="color-2">'.$chinaAdd[ 'noInfect' ].'</span>';}else{$adddata2='较昨日<span class="color-2">+'.$chinaAdd[ 'noInfect' ].'</span>';}
				if($chinaAdd[ 'nowConfirm' ]==0){$adddata3='数据待更新';	}else if($chinaAdd[ 'nowConfirm' ]<0){$adddata3='较昨日<span class="color-3">'.$chinaAdd[ 'nowConfirm' ].'</span>';}else{$adddata3='较昨日<span class="color-3">+'.$chinaAdd[ 'nowConfirm' ].'</span>';}
				if($chinaAdd[ 'confirm' ]==0){$adddata4='数据待更新';	}else if($chinaAdd[ 'confirm' ]<0){$adddata4='较昨日<span class="color-4">'.$chinaAdd[ 'confirm' ].'</span>';}else{$adddata4='较昨日<span class="color-4">+'.$chinaAdd[ 'confirm' ].'</span>';}
				if($chinaAdd[ 'dead' ]==0){$adddata5='数据待更新';	}else if($chinaAdd[ 'dead' ]<0){$adddata5='较昨日<span class="color-5">'.$chinaAdd[ 'dead' ].'</span>';}else{$adddata5='较昨日<span class="color-5">+'.$chinaAdd[ 'dead' ].'</span>';}
				if($chinaAdd[ 'heal' ]==0){$adddata6='数据待更新';	}else if($chinaAdd[ 'heal' ]<0){$adddata6='较昨日<span class="color-6">'.$chinaAdd[ 'heal' ].'</span>';}else{$adddata6='较昨日<span class="color-6">+'.$chinaAdd[ 'heal' ].'</span>';}
				switch ( $matches[ 1 ][ $i ] ) {

					case 'data1':
						$zcontent = str_replace( '{zzz:data1}', $chinaTotal[ 'importedCase' ], $zcontent );
						break;
					case 'adddata1':
						$zcontent = str_replace( '{zzz:adddata1}', $adddata1, $zcontent );
						break;
					case 'data2':
						$zcontent = str_replace( '{zzz:data2}', $chinaTotal[ 'noInfect' ], $zcontent );
						break;
					case 'adddata2':
						$zcontent = str_replace( '{zzz:adddata2}', $adddata2, $zcontent );
						break;
					case 'data3':
						$zcontent = str_replace( '{zzz:data3}', $chinaTotal[ 'nowConfirm' ], $zcontent );
						break;
					case 'adddata3':
						$zcontent = str_replace( '{zzz:adddata3}', $adddata3, $zcontent );
						break;
					case 'data4':
						$zcontent = str_replace( '{zzz:data4}', $chinaTotal[ 'confirm' ], $zcontent );
						break;
					case 'adddata4':
						$zcontent = str_replace( '{zzz:adddata4}', $adddata4, $zcontent );
						break;
					case 'data5':
						$zcontent = str_replace( '{zzz:data5}', $chinaTotal[ 'dead' ], $zcontent );
						break;
					case 'adddata5':
						$zcontent = str_replace( '{zzz:adddata5}', $adddata5, $zcontent );
						break;
					case 'data6':
						$zcontent = str_replace( '{zzz:data6}', $chinaTotal[ 'heal' ], $zcontent );
						break;
					case 'adddata6':
						$zcontent = str_replace( '{zzz:adddata6}', $adddata6, $zcontent );
						break;
	...
	function parserOtherLabel( $zcontent ) {
		$pattern = '/\{(formatdate|cutpic|leftstr|hidestr|count|region|html2txt):([\s\S]*?)}/';
		if ( preg_match_all( $pattern, $zcontent, $matches ) ) {            
			$count = count( $matches[ 0 ] );
			for ( $i = 0; $i < $count; $i++ ) {
				$params = parserParam( $matches[ 2 ][ $i ] );
				switch ( $matches[ 1 ][ $i ] ) {
					...
					case 'region':
						...
						break;			
					case 'html2txt':
						$zcontent = str_replace( $matches[ 0 ][ $i ], html_txt( $matches[ 2 ][ $i ] ), $zcontent );
						break;				
				}
			}
		}
		...
		if ( preg_match_all( $pattern, $zcontent, $matches ) ) {
			...
			for ( $i = 0; $i < $count; $i++ ) {
				switch($matches[ 1 ][ $i ] ){
					...
					break;	
					case 'location':	
					if (get_session('s_lid')==2) {
        			$remark = 'en/';
        			$home=  parserParam($matches[ 2 ][ $i ],'home','Home');
        			}else{
        			$remark = '/';
					...
					$style=  parserParam($matches[ 2 ][ $i ],'style','<span> > </span>');  
					$crumbs= is_array(G('crumbs'))  ?implode($style,G('crumbs')) :  G('crumbs') ;
					$zcontent = str_replace( $matches[ 0 ][ $i ], "<a href='" . SITE_PATH .$remark. "'>".$home.$style."</a>" . $crumbs, $zcontent );
					break;	
					default:
					$zcontent = str_replace( $matches[ 0 ][ $i ], '', $zcontent );
				}				
