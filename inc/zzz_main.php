...
function cleft( $str, $start = 0, $length = 0 ,$append =0) {
	...
	$newstr = mb_substr($str, $start, $length, 'utf8');	
...
function getform( $name, $source = 'both', $type = NULL, $default = NULL ) {
	switch ( $source ) {
		...
		case 'get':
			...
			break;
		case 'cookie':
			$data = safe_key(_REQUEST($name),20);
            if($data) {
                 setcookie( $name,$data ) ;
            }else{
...
function danger_key($s,$type=0) {
	if($type==1){
        ...
        $s =str_replace( array("php","\0", "%00", "\r","<", ">","'", '"', "{","}", "%3C", "%3E"), '', $s);
	}
	$str= $s;
...
function white_ip($ip) {
	...
	if ( !$test_success )error( '禁止访问,您不允许访问本站，IP不在白名单范围内' );
}
...
function getlocation() {
	...
	$url = danger_key(str_replace(array(conf('siteext'),'.php'),'',$_SERVER[ 'REQUEST_URI' ]),1);
...
function checklocation( $q, $p = NULL ) {   
	...
	$arr2 = array( 'order','shopcart','shopconfirm', 'user','userorder','shoporder','shopcomment','pointsorder', 'form',  conf('wappath'), 'sitemap', 'sitexml' );
	$arr3 = load_model();
	if ( in_array( $q, $arr1 ) ) {
		$p = sub_right( $p, '/' );
		$sid = arr_split($p,'_',0);
